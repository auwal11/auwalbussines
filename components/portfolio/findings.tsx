      ...
      <div className="mx-auto max-w-6xl px-4 py-24 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 gap-10 lg:grid-cols-12">
          <Reveal className="lg:col-span-4">
            ...
            <div className="mt-6 inline-flex items-center gap-2 rounded-full border border-[rgba(0,229,255,0.1)] bg-[#0F0E1A]/60 px-3 py-1.5 font-mono text-[11px] text-[#94A3B8]">
              <Lock className="h-3 w-3 text-[#FF6B00]" />
          ...
          <div className="lg:col-span-8 lg:pl-8 lg:border-l lg:border-zinc-900">
            <Reveal>
              ...
              <ul className="mt-4 divide-y divide-zinc-900 border-y border-zinc-900">
                {PLATFORMS.map((p, i) => (
                  <li
                    key={p.name}
                    className="group grid grid-cols-12 items-center gap-3 px-1 py-4 transition hover:bg-[#FF6B00]/[0.02]"
                  >
                    <span className="col-span-1 font-mono text-[11px] tracking-widest text-[#64748B]">
                      {String(i + 1).padStart(2, "0")}
                    </span>
                    <span className="col-span-5 font-mono text-base font-semibold text-[#F8FAFC] group-hover:text-[#FF6B00] transition-colors sm:col-span-4">
                      {p.name}
                    </span>
                      ...
                      <span className="inline-flex items-center gap-1.5 font-mono text-[10px] uppercase tracking-wider text-[#94A3B8]">
                        <span
                          className={`h-1.5 w-1.5 rounded-full ${
                            p.status === "Active" ? "bg-[#FF6B00]" : "bg-zinc-500"
            ...
            <Reveal delayMs={120}>
              <p className="mt-12 font-mono text-[10px] uppercase tracking-[0.3em] text-[#64748B]">
                Research focus
              </p>
              <ul className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2">
                {DOMAINS.map((d) => (
                  <li
                    key={d.label}
                    className="group rounded-lg border border-[rgba(0,229,255,0.1)]/80 bg-[#0F0E1A]/30 p-5 transition hover:border-amber-400/40 hover:bg-[#0F0E1A]/60"
                  >
                    <p className="font-mono text-sm font-semibold text-[#F8FAFC] group-hover:text-[#FF6B00] transition-colors">
                      {d.label}
                    </p>
